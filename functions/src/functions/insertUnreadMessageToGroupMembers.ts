...
export const insertGroupMembersUnreadMessageSegmnent = functions
  .firestore
  .document(path)
  .onCreate(async (snap, context) => {
    ...
    room
      ...
      .forEach(collectionPath => {
        batch
          .set(firestore.collection(collectionPath).doc(
            getRandomNodeId(DISTRIBUTED_COUNTER_NODE_SIZE)),
            { [snap.id]: message },
            { merge: true }
          )
      })
...
async function fetchRoomById(roomId: Id) {
  const snap = await firestore.collection(getRoomPath()).doc(roomId).get()
